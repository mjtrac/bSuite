...
class TemplateReflectedInBallotTest {
    ...
    @Test
    @DisplayName("Legacy Quill-authored headerHtml (class=\"ql-align-*\") still renders "
        + "correctly — blBuilder's editor no longer produces these classes, but "
        + "historical BallotDesignTemplate rows saved by the old Quill editor still "
        + "have them, and drawHeaderZone's class-to-inline-style conversion must "
        + "keep handling them")
    void legacyQuillAlignClassesStillRender() throws Exception {
        template.setHeaderHtml(
            "<div style=\"font-family:Helvetica;padding:4px 0\">" +
            "<p class=\"ql-align-center\" style=\"font-size:16pt;font-weight:bold\">" +
            "OFFICIAL BALLOT</p>" +
            "<p class=\"ql-align-right\" style=\"font-size:9pt\">{jurisdictionName}</p>" +
            "<p class=\"ql-align-justify\" style=\"font-size:9pt\">{electionName}</p>" +
            "</div>");
        byte[] legacyPdf = ballotService.generateBallot(combo, template, testUser, 1);
        assertThat(new String(legacyPdf, 0, 4)).isEqualTo("%PDF");

        // Same content but with the alignment already expressed as plain inline
        // style (what a hand-typed, no-Quill textarea produces) should render
        // to something extremely close — proving the ql-* classes were actually
        // converted rather than silently dropped by html2pdf.
        template.setHeaderHtml(
            "<div style=\"font-family:Helvetica;padding:4px 0\">" +
            "<p style=\"text-align:center;font-size:16pt;font-weight:bold\">" +
            "OFFICIAL BALLOT</p>" +
            "<p style=\"text-align:right;font-size:9pt\">{jurisdictionName}</p>" +
            "<p style=\"text-align:justify;font-size:9pt\">{electionName}</p>" +
            "</div>");
        byte[] inlineStylePdf = ballotService.generateBallot(combo, template, testUser, 1);
        assertThat(new String(inlineStylePdf, 0, 4)).isEqualTo("%PDF");

        // And a version with no alignment at all should differ from both —
        // confirming the ql-* classes changed layout rather than being a no-op.
        template.setHeaderHtml(
            "<div style=\"font-family:Helvetica;padding:4px 0\">" +
            "<p style=\"font-size:16pt;font-weight:bold\">OFFICIAL BALLOT</p>" +
            "<p style=\"font-size:9pt\">{jurisdictionName}</p>" +
            "<p style=\"font-size:9pt\">{electionName}</p>" +
            "</div>");
        byte[] unalignedPdf = ballotService.generateBallot(combo, template, testUser, 1);

        assertThat(legacyPdf).isNotEqualTo(unalignedPdf);
        assertThat(inlineStylePdf).isNotEqualTo(unalignedPdf);
    }
