...
/**
 * Generates a real ballot PDF+YAML for a single 5-candidate RANKED_CHOICE
 * contest, for the RCV bulk-tie-elimination scenario also covered by
 * RcvTabulationServiceTest.testFiveCandidateBulkTieElimination() (same
 * expected 3-round result, but that test calls runIrv() directly with a
 * synthetic ballot list — this generates the real ballot artifacts so the
 * full mark-detection -> scan -> tabulate pipeline can be exercised too,
 * not just the pure IRV algorithm).
 *
 * Mirrors TestElectionBuilder's headless, no-web-server pattern.
 *
 * Usage: mvn -q -o exec:java -Dexec.mainClass=com.mjtrac.builderui.RcvFiveCandidateBuilder
 ...
 */
public class RcvFiveCandidateBuilder {

    private static final String JURISDICTION_NAME = "RCV Five-Candidate Test County";
    private static final String ELECTION_NAME = "RCV Five-Candidate Test Election";
    private static final String REGION_NAME = "Precinct 1";
    private static final String BALLOT_TYPE_NAME = "Precinct";
    private static final String CONTEST_TITLE = "Five-Candidate RCV Test";
    private static final String PRINT_USER = "rcv-test-harness";
    ...
    static void run(String[] args) throws Exception {
        ...
        try {
            String outPath = ctx.getEnvironment().getProperty("test-election.out", "rcv_election_data.json");
            ...
            if (contests.isEmpty()) {
                ...
                contest.setMaxRankChoices(5);
                contest.setVotingMethod(Contest.VotingMethod.RANKED_CHOICE);
                contest.setInstructions("Rank candidates 1-5.");
            ...
            System.out.println("RcvFiveCandidateBuilder: wrote " + outPath + " (combination "
                + combination.getId() + ")");
