...
/**
 * Generates a real ballot PDF+YAML for a single MEASURE contest requiring a
 * 60% (not the default 50%) supermajority to pass — the "tax measure"
 * scenario named directly in the percentToWin feature request. Mirrors
 * RcvFiveCandidateBuilder's headless, no-web-server pattern exactly, but for
 * a 2-choice, maxChoices=1 contest with percentToWin=60.0 instead of a
 * RANKED_CHOICE contest.
 *
 * Usage: mvn -q -o exec:java -Dexec.mainClass=com.mjtrac.builderui.PercentThresholdBuilder
 ...
 */
public class PercentThresholdBuilder {

    private static final String JURISDICTION_NAME = "Percent Threshold Test County";
    private static final String ELECTION_NAME = "Percent Threshold Test Election";
    private static final String REGION_NAME = "Precinct 1";
    private static final String BALLOT_TYPE_NAME = "Precinct";
    private static final String CONTEST_TITLE = "Measure A — 60% Threshold Test";
    private static final String PRINT_USER = "percent-test-harness";
    ...
    static void run(String[] args) throws Exception {
        ...
        try {
            String outPath = ctx.getEnvironment().getProperty("test-election.out", "percent_election_data.json");
            ...
            if (contests.isEmpty()) {
                ...
                contest.setVotingMethod(Contest.VotingMethod.MEASURE);
                contest.setPercentToWin(60.0);
                contest.setInstructions("Vote Yes or No.");
            ...
            System.out.println("PercentThresholdBuilder: wrote " + outPath + " (combination "
                + combination.getId() + ")");
